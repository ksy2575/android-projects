...
public abstract class CameraActivity extends AppCompatActivity
  ...
  String[] nameList1_1 = new String[]{"chair", "clock", "eraser",
          "mouse", "pen", "shoes",
...
//              if(!isInterrupted())Log.d("jdada", "첫번"+answer1+levelList_1[currentpoistion]);
            }
            else if(answer2.equals(levelList_1[currentpoistion])){//두번째 단어가 정답일 시
              string = answer2;
              isCorrectAnswer = true;
//              if(!isInterrupted())Log.d("jdada", "두번");
            }
            else{
              string = answer1;//둘 다 아닐시(오답)
              isCorrectAnswer = false;
//              if(!isInterrupted())Log.d("jdada", "세번"+answer1+levelList_1[currentpoistion]);
            }
